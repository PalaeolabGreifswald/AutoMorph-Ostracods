...
def extractOutline(settings, image_name, image, run3dmorph):
    ...
    '''
    if not run3dmorph:
        print(f"INFO: Extracting outline for {image_name}")
    size = image.shape
    ...
    try:
        ...
        if settings['save_intermediates']:
            save.saveIntermediates(settings, image_open, image_name, f"1_{tagDict['1']}")

        # RGB filter
        image_rgb = filters.rgbFilter(image_open)
        if settings['save_intermediates']:
            save.saveIntermediates(settings, image_rgb, image_name, f"2_{tagDict['2']}")

        # Contrast adjustment
        image_contrast = filters.contrastAdjust(image_rgb, settings['contrast_adjustment'])
        if settings['save_intermediates']:
            save.saveIntermediates(settings, image_contrast, image_name, f"3_{tagDict['3']}")

        # Grayscale conversion
        image_gray = filters.rgb2gray(image_contrast)
        if settings['save_intermediates']:
            save.saveIntermediates(settings, image_gray, image_name, f"4_{tagDict['4']}")

        # Black & White conversion
        image_bw = filters.convertBW(image_gray, image_name, settings['threshold_adjustment'])
        if settings['save_intermediates']:
            save.saveIntermediates(settings, image_bw, image_name, f"5_{tagDict['5']}")

        # Created hole-filled version and -unfilled version (the latter is for
        # run3dmorph)
        image_filled = filters.fillHoles(image_bw)
        if settings['save_intermediates']:
            save.saveIntermediates(settings, image_filled, image_name, f"6_{tagDict['6']}")
        if run3dmorph:
        ...
        if settings['save_intermediates']:
            save.saveIntermediates(settings, image_border, image_name, f"7_{tagDict['7']}")
        if run3dmorph:
        ...
        if settings['save_intermediates']:
            save.saveIntermediates(settings, image_clean, image_name, f"8_{tagDict['8']}")
        if run3dmorph:
        ...
        if settings['save_intermediates']:
            save.saveIntermediates(settings, edge_unsmoothed, image_name, f"9_{tagDict['9']}")
        ...
        if not run3dmorph:
            ...
            if settings['save_intermediates']:
                save.saveIntermediates(settings, edge_smoothed, image_name, f"10_{tagDict['10']}")
        
        print('')
    except Exception as e:
        print(f"ERROR: Outline extraction failed; check intermediates and/or input parameters. Exception: {e}")
