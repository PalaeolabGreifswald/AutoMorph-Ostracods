...
def convertBW(image, image_name, threshold_adjustment):
    ...
    if threshold + threshold_adjustment >= 0:
        ...
        print(f'INFO: Threshold adjustment value too large! Using automatic value for {image_name}')
    return image_bw


def fillHoles(image):
    '''
    Fills holes contained within a binary object.
    '''
    image_filled = ndimage.binary_fill_holes(image / 255.0).astype("uint8") * 255
    return image_filled
...
def clearBorder(image, size):
    ...
    for x in range(w):
        cv2.floodFill(image_noise, mask, (x, 0), 0)
        cv2.floodFill(image_noise, mask, (x, h - 1), 0)
    for y in range(h):
        cv2.floodFill(image_noise, mask, (0, y), 0)
        cv2.floodFill(image_noise, mask, (w - 1, y), 0)
