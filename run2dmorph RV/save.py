...
def saveObject2D(dataFrame, sampleID, objectID, measures, height, width, aspectRatio):
    ...
    dataFrame = pd.concat([dataFrame, toAdd], ignore_index=True)  # Updated for compatibility with pandas 1.0+
    return dataFrame
...
def saveCoordinates(settings, coordinates, sampleID, objectID, object_name, tag):
    ...
    save = pd.DataFrame(columns=['SampleID', 'ObjectID', 'x', 'y'])
...
def saveMBBFigure(settings, mbb, contour, aspect_ratio, object_name):
    ...
    '''
    # Close minimum bounding box by appending first coordinate to end 
