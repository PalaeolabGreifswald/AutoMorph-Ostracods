...
# sorting outline points clockwise, starting from the far right
def sortCounterClockwiseFromRight(coords):
    ...
    rightmost_index = np.argmax(sorted_coords[:, 0])
    
    rotated_coords = np.roll(sorted_coords, -rightmost_index, axis=0)
...
def extractCoordinates(edge, downsample, num_points, region_properties, major_axis_length):
    ...
    for i in range(len(coords)):
        ...
        if i == int(0.75 * num_coords):
            processing = np.vstack([processing, coords[0]])
            

    ordered = np.vstack(ordered)  # Put points into correct data structure
    ordered = sortCounterClockwiseFromRight(ordered)
