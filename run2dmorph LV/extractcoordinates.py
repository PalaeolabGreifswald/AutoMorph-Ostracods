...
# sorting outline points clockwise, starting from the far left
def sortClockwiseFromLeft(coords):
    ...
    leftmost_index = np.argmin(sorted_coords[:, 0])

    rotated_coords = np.roll(sorted_coords, -leftmost_index, axis=0)
...
def extractCoordinates(edge, downsample, num_points, region_properties, major_axis_length):
    ...
    ordered = sortClockwiseFromLeft(ordered)
