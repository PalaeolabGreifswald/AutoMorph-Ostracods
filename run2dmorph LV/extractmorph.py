...
def getRegionProps(binary_image):
    '''
    Returns region properties for object identified by outline extraction,
    both original (unsmoothed) and smoothed.
    '''
    labels, a = ndimage.label(binary_image)
    properties = regionprops(labels)[0]  # Get properties for the first labeled region
...
def extractMorphology(original_props, smoothed_props):
    ...
    rugosity = original_props.perimeter / smoothed_props.perimeter  # Division is true division in Python 3
